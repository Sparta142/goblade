...
func TestReadBundle_CompressedIpc(t *testing.T) {
	assert := assert.New(t)
	...
	assert.Equal(IpcMagicString, string(b.Magic[:])) // Should be enforced by ReadBundle()
	assert.EqualValues(1624314020072, b.Epoch)
	assert.EqualValues(266, b.Length)
	assert.EqualValues(0, b.ConnectionType)
	assert.EqualValues(1, b.SegmentCount)
	...
	assert.Len(b.Segments, int(b.SegmentCount)) // Sanity check
	...
	// The entire reader should have been consumed
	assert.EqualValues(0, r.Len())
}

func TestReadBundle_NonCompressedIpc(t *testing.T) {
	assert := assert.New(t)
	r := bytes.NewReader([]byte{
		0x52, 0x52, 0xa0, 0x41, 0xff, 0x5d, 0x46, 0xe2,
		0x7f, 0x2a, 0x64, 0x4d, 0x7b, 0x99, 0xc4, 0x75,
